...
`getactivity` is paginated by Withings at 200 records per response. When a page reports `more`, the binary re-requests with the returned `offset` and keeps going until exhausted, upserting every page before advancing the cursor. This matters on large back-fills after a long gap — without it, everything past the first 200 daily records would be silently dropped. The loop is bounded at 50 pages (10 000 daily records ≈ 27 years) as a safety net against the known Withings bug where `more` stays `true` indefinitely; if that bound is hit — or a page reports `more` without an `offset` — the sync **aborts with an error and does not advance the activity cursor**, so the run fails loudly and the next run retries the same window rather than silently skipping records.
