...
`getactivity` is paginated by Withings at 200 records per response. When a page reports `more`, the binary re-requests with the returned `offset` and keeps going until exhausted (capped at 50 pages/run), upserting every page before advancing the cursor. This matters on large back-fills after a long gap — without it, everything past the first 200 daily records would be silently dropped.
